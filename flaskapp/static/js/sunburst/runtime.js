...
function w(n, i, o, a) {
    let s, l, u, c, d = t(n);
    n instanceof Map ? (s = `Map(${n.size})`, l = y) : n instanceof Set ? (s = `Set(${n.size})`, l = g) : d ? (s = `${n.constructor.name}(${n.length})`, l = x) : (c = m(n)) ? (s = `Immutable.${c.name}${"Record"===c.name?"":`(${n.size})`}`, d = c.arrayish, l = c.arrayish ? C : c.setish ? E : S) : a ? (s = f(n), l = N) : (s = f(n), l = P);
...
function O(e, n, i, o) {
    let a, s, l, u, c = t(e);
    if (e instanceof Map ? (a = `Map(${e.size})`, s = T) : e instanceof Set ? (a = `Set(${e.size})`, s = A) : c ? (a = `${e.constructor.name}(${e.length})`, s = R) : (u = m(e)) ? (a = `Immutable.${u.name}${"Record"===u.name?"":`(${e.size})`}`, c = u.arrayish, s = u.arrayish ? U : u.setish ? $ : F) : (a = f(e), s = D), n) {
        const t = document.createElement("span");
